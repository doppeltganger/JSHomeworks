/*
task description: Есть три объекта: users, posts и comments. Написать код функции getPostComments(postId), которая возвращает массив всех комментариев к посту с идентификатором postId (с именем автора комментария и текстом комментария).
*/
...
function getCurrentPostComments (postId) {
  ...
  for (let id in comments) {
    if (comments[id]['postId'] == postId) {
      result.push({
        author: users[comments[id]['author']]['name'],
